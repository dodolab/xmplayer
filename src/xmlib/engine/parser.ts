...
export class XMParser {
	...
	private parsePatterns (buffer: Uint8Array, file: XMFile) {
		...
		for (let i = 0; i < file.patternsNum; i++) {
			...
			const datalen = this.readWord(buffer, this.currentOffset + 7) // size of pattern data
			this.currentOffset += this.readDWord(buffer, this.currentOffset) // jump over length of pattern header

			let j = 0
			let k = 0
			while (j < datalen) {
				const c = buffer[this.currentOffset + j++]
				if (c & 128) {
	...
	private parseInstrumentEnvelopes (buffer: Uint8Array, instrument: Instrument) {
		...
		for (let i = 0; i < 12; i++) {
			...
			const panXCoord = this.readWord(buffer, this.currentOffset + 0xc1 + i * 4)
			const panYCoord = this.readWord(buffer, this.currentOffset + 0xc1 + i * 4 + 2)
	...
	private parseInstrumentSamples (buffer: Uint8Array, instrument: Instrument) {
		...
		for (let j = 0; j < instrument.samplesNum; j++) {
			const datalen = this.readDWord(buffer, this.currentOffset + 0)
